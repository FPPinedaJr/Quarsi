...
if ($input !== '') {
    ...
    $query = "
        ...
        WHERE (f_name LIKE ? OR l_name LIKE ?) AND user.is_admin <> 1
        ORDER BY is_superuser DESC, is_officer DESC, user.year, user.f_name;
    ";
...
if (count($students) > 0) {
    foreach ($students as $student): ?>
        <tr id="student-<?php echo $student['iduser'] ?>" data-student_no="<?php echo $student['student_no'] ?>"
            ...
            data-email="<?php echo $student['email'] ?>" data-profile_pic="<?= base64_encode($student['profile_pic']) ?>"
            ...
            <td class="flex items-center py-2 pl-3"><img data-src="<?php if ($student['profile_pic']) {
                ...
                <p class="<?php if ($student['is_superuser'] == 0 && $student['is_officer'] == 1) {
                    echo 'text-blue-600';
...
if (count($students) > 0): ?>
    <?php foreach ($students as $student): ?>
        ...
        <div id="student-<?php echo $student['iduser'] ?>" data-student_no="<?php echo $student['student_no'] ?>"
            ...
            class="flex student-div items-center p-1 border-2 border-gray shadow-md w-full h-36 rounded-md bg-[#d9f0ea] my-3 overflow-hidden"
            onclick="showEditStudentModal(<?php echo $student['iduser'] ?>)">
            <div class="flex items-center justify-center w-1/3 h-full">
                <img data-src="data:image/jpeg;base64,<?= base64_encode($student['profile_pic']) ?>" alt="profile picture"
                    class="w-16 h-16 border border-gray-200 rounded-full lozad">
            </div>
            <div class="flex flex-col justify-center w-2/3 h-full p-1 pl-2 text-nowrap">
                ...
                <p class=""><?php echo $student['program'] ?>         <?php echo $student['year'] ?> </p>
            </div>
        </div>
    <?php endforeach; ?>
    </div>
