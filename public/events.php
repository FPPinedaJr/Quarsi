...
if (!$_SESSION["logged_in"] || !($_SESSION['is_superuser'] == 1 || $_SESSION['is_admin'] == 1)) {
    ...
    $stmt3 = $pdo->prepare("
        SELECT o.short_name as organization, year, f_name, l_name, iduser
        FROM user u
        INNER JOIN organization o ON u.organization = o.idorganization
        WHERE is_admin != 1 
        ORDER BY organization, year, l_name, f_name
    ...
    <body class="justify-center w-screen min-h-screen mt-20 overflow-x-hidden ">
        ...
        <main class="flex justify-center w-full min-h-screen px-3 py-2 h-fit">
            ...
            <div class="justify-center hidden w-full my-4 md:flex h-fit">
                <table class="w-full md:w-2/3">
                    ...
                    <?php foreach ($events as $event): ?>
                        <tr id="event-<?= $event['idevent'] ?>" data-idevent="<?= $event['idevent'] ?>"
            ...
            <!-- mobile view -->
            <div class="flex flex-col justify-center w-full gap-5 my-4 md:hidden h-fit">
                <?php foreach ($events as $event): ?>
                    <div id="event-<?= $event['idevent'] ?>" data-idevent="<?= $event['idevent'] ?>"
                        data-name="<?= $event['name'] ?>" data-date="<?= $event['date'] ?>"
                        data-log_time="<?= $event['log_time'] ?>" data-morning_in="<?= $event['morning_in'] ?>"
                        data-morning_out="<?= $event['morning_out'] ?>" data-afternoon_in="<?= $event['afternoon_in'] ?>"
                        data-afternoon_out="<?= $event['afternoon_out'] ?>" data-users="<?= $event['invited_users'] ?>"
                        ...
                        <div class="flex flex-col justify-center w-3/5 h-full">
                            <div class="pl-2 text-lg font-semibold text-nowrap"><?= $event['name'] ?></div>
        ...
        <div id="edit_event_modal"
            ...
            <div id="edit_event_modal_main" class="relative flex flex-col w-5/6 h-fit md:w-[25rem]">
                <div class="relative flex items-center justify-center w-full h-12 text-center bg-teal-700 md:h-16">
                    ...
                    <!-- Options -->
                    <div class="absolute p-1 right-3 top-3 w-fit h-fit">
                        <div class="relative">
                            <i onclick="toggleEventOptions()"
                                class="text-xl text-white cursor-pointer fa-solid fa-ellipsis hover:text-emerald-400"></i>
                            <div id="eventOptions"
                                class="absolute flex flex-col invisible w-40 text-sm bg-white border border-gray-500 rounded-lg -right-4 top-6">
                                <div id="inviteBtn"
                                ...
                                <a id="eventAttendance" href="" class="">
                                    <div id="attendanceBtn"
                                        class="flex items-center w-full px-2 py-1 rounded-b-lg cursor-pointer hover:bg-gray-200">
                                        <div class="flex items-center justify-center w-6 h-full mr-2">
                                            <i class="mr-1 fa-solid fa-clipboard-list"></i>
                                        </div>
                                        View Attendance
                                    </div>
                                </a>
        ...
        <div id="delete_event_modal"
            ...
            <div id="delete_event_modal_main"
                ...
                <div class="flex flex-col w-full h-auto p-2 text-md">
                    <p class="font-semibold text-emerald-700">Are you sure to delete event "<span
                            id="event_to_delete"></span>"?</p>
                </div>
                ...
                <div id="invite_modal_main" class="w-full h-full overflow-y-auto text-lg bg-white">
                    ...
                    foreach ($students as $student) {
                        ...
                        if ($student['year'] !== $currentYear) {
                            ...
                            echo '<i class="mr-3 text-teal-700 cursor-pointer fa-solid fa-caret-right year-dropdown"></i><label><input type="checkbox" class="year-checkbox"> <span class="font-semibold">' . $year_dict[$currentYear] . '</span></label>
                            ';
        ...
        <div id="edit_invite_modal"
            ...
            <div id="edit_invite_modal_main"
                class="w-10/12 overflow-x-hidden overflow-y-auto text-lg bg-white md:w-1/3 h-2/3">
                <form id="edit_invite_students_form" action="./includes/crud_invite.php" type="button" method="POST"
                    ...
                    <div class="flex w-full p-1 my-2 ml-2 text-xl font-semibold h-fit text-zinc-700">
                        <p>Invited Students: <span id="invited-students" class="text-teal-500"></span></p>
                    </div>
                    <?php
                    $currentProgram = '';
                    $currentYear = '';
        ...
        <div id="switch_modal"
            ...
            <div id="switch_modal_main" class="rounded-lg w-3/4 md:w-1/4 h-36 flex flex-col bg-[#fbfcf8] p-2">
                ...
                <div class="w-full h-auto bg-[#fbfcf8] text-teal-800 flex flex-wrap">
                    <p>
                        Are you sure to turn <span id="changelog_status" class="font-semibold"></span> <span id="changelog"
                            class="italic"></span> log?
                    </p>
                </div>
        ...
        <!-- Confirm Invite -->
        <div id="confirm_invite"
            class="fixed top-0 left-0 z-40 flex items-center justify-center invisible w-full h-full bg-gray-500/30 backdrop-blur-sm">
            <div id="confirm_invite_main" class="rounded-lg w-3/4 md:w-1/4 h-36 flex flex-col bg-[#fbfcf8] p-2">
                <div class="w-full pt-2 mb-2 text-lg font-semibold text-teal-800 border-b border-teal-700 h-fit">Confirm
                    Invite</div>
                <div class="w-full h-auto bg-[#fbfcf8] text-teal-800 flex flex-wrap">
                    <p>
                ...
                <div class="flex justify-center w-full mt-auto mb-1">
                    <button onclick="hideConfirmInviteModal()" type="button"
                        class="px-2 py-1 text-teal-800 rounded hover:text-teal-500 hover:underline">Cancel</button>
                    <button class="px-2 py-1 ml-8 text-white bg-teal-800 rounded hover:bg-teal-500"
                        onclick="confirmInvite()">Confirm</button>
                </div>
            </div>
        </div>

        <!-- Invite Error -->
        <div id="confirm_invite_error"
            class="fixed top-0 left-0 z-40 flex items-center justify-center invisible w-full h-full bg-gray-500/30 backdrop-blur-sm">
            <div id="confirm_invite_error_main" class="rounded-lg w-3/4 md:w-1/4 h-36 flex flex-col bg-[#fbfcf8] p-2">
                <div class="w-full pt-2 mb-2 text-lg font-semibold text-teal-800 border-b border-teal-700 h-fit">Invite
                    Error</div>
                <div class="w-full h-auto bg-[#fbfcf8] text-teal-800 flex flex-wrap">
                    <p>
                ...
                <div class="flex justify-center w-full mt-auto mb-1">
                    <button onclick="hideConfirmInviteErrorModal()" type="button"
                        class="px-2 py-1 text-white bg-teal-700 rounded">Okay</button>
                </div>
            </div>
        </div>

        <!-- Confirm Update Invite -->
        <div id="confirm_update_invite"
            class="fixed top-0 left-0 z-40 flex items-center justify-center invisible w-full h-full bg-gray-500/30 backdrop-blur-sm">
            <div id="confirm_update_invite_main" class="rounded-lg w-3/4 md:w-1/4 h-36 flex flex-col bg-[#fbfcf8] p-2">
                ...
                <div class="flex justify-center w-full mt-auto mb-1">
                    <button onclick="hideConfirmEditInviteModal()" type="button"
                        class="px-2 py-1 text-teal-800 rounded hover:text-teal-500 hover:underline">Cancel</button>
                    <button class="px-2 py-1 ml-8 text-white bg-teal-800 rounded hover:bg-teal-500"
                        onclick="confirmEditInvite()">Confirm</button>
                </div>
            </div>
        </div>
        ...
        <div id="notif_invite"
            class="fixed flex items-center justify-between invisible w-1/2 h-10 text-xs text-teal-700 -translate-x-1/2 bg-white border border-teal-500 rounded-md shadow top-30 left-1/2 md:w-1/5 md:text-sm">
            <p class="flex-1 text-center">
                Successfully invited <span id="success_invite_count" class="font-bold"></span> students
            </p>
            <button class="h-full p-2 rounded-md hover:bg-gray-200">
    ...
    <script>
        ...
        function showEditInviteModal() {
            ...
            $('#edit_invite_modal_main').find('.year-checkbox').each(function () {
                ...
                var $studentCheckboxes = $yearCheckbox.closest('.year').find('.student-checkbox');
        ...
        function setupInviteModal(modalSelector) {
            // Collapse all years by default
            var $dropdowns = $(modalSelector + ' .year-dropdown');
            $dropdowns.each(function () {
                var $container = $(this).closest('.year').find('> div');
                $(this).removeClass('fa-caret-down').addClass('fa-caret-right');
                $container.addClass('hidden');
            });

            // Toggle year dropdown
            $(modalSelector).on('click', '.year-dropdown', function () {
                var $dropdown = $(this);
                var $container = $dropdown.closest('.year').find('> div'); // student container
            ...
            // Toggle program checkbox → select/deselect all years + students
            $(modalSelector).on('change', '.program-checkbox', function () {
                var $program = $(this).closest('.program-group');
                var $years = $program.find('.year-checkbox');
                var $students = $program.find('.student-checkbox');

                $years.prop('checked', this.checked);
                $students.prop('checked', this.checked);
            });

            // Toggle year checkbox → select/deselect all students
            $(modalSelector).on('change', '.year-checkbox', function () {
                var $year = $(this).closest('.year');
                var $students = $year.find('.student-checkbox');
                $students.prop('checked', this.checked);
            });

            // When a student is clicked → update year checkbox
            $(modalSelector).on('change', '.student-checkbox', function () {
                var $year = $(this).closest('.year');
                var $students = $year.find('.student-checkbox');
                var $yearCheckbox = $year.find('.year-checkbox');

                var total = $students.length;
                var checked = $students.filter(':checked').length;

                $yearCheckbox.prop('checked', total === checked);
            });
        }

        $(document).ready(function () {
            setupInviteModal('#invite_modal_main');
            setupInviteModal('#edit_invite_modal_main');
        });
    </script>
<?php } ?>